...
def upgrade() -> None:
    ...
    op.add_column('user', sa.Column('user_status', sa.String(), nullable=False, server_default='ACTIVE'))
    op.add_column('user', sa.Column('waitlist_interest', sa.String(), nullable=True))
    op.add_column('user', sa.Column('waitlist_joined_date', sa.String(), nullable=True))
    op.add_column('user', sa.Column('waitlist_activated_date', sa.String(), nullable=True))
    ...
    op.execute("UPDATE \"user\" SET user_status = 'ACTIVE' WHERE user_status IS NULL")
    ...
    op.create_table(
        ...
        sa.Column('total_registrations', sa.Integer(), nullable=False, server_default='0'),
        ...
        sa.Column('batch_delay_seconds', sa.Integer(), nullable=False, server_default='2'),
    ...
    op.create_index('ix_waitlist_config_waitlist_uuid', 'waitlist_config', ['waitlist_uuid'])
    op.create_index('ix_waitlist_config_org_status', 'waitlist_config', ['org_id', 'status'])
    op.create_index('ix_waitlist_config_launch_datetime', 'waitlist_config', ['launch_datetime'])
    ...
    op.create_table(
        ...
        sa.Column('creation_date', sa.String(), nullable=False),
        sa.Column('update_date', sa.String(), nullable=False),
        sa.ForeignKeyConstraint(['waitlist_config_id'], ['waitlist_config.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create indexes for waitlist_email_log
    op.create_index('ix_waitlist_email_log_user_id', 'waitlist_email_log', ['user_id'])
    op.create_index('ix_waitlist_email_log_waitlist_config_id', 'waitlist_email_log', ['waitlist_config_id'])
    op.create_index('ix_waitlist_email_log_status', 'waitlist_email_log', ['email_sent'])
    ...
    op.create_index('ix_wcp_waitlist_config_id', 'waitlist_course_preference', ['waitlist_config_id'])
    op.create_index('ix_wcp_course_id', 'waitlist_course_preference', ['course_id'])
    op.create_index('ix_wcp_unique', 'waitlist_course_preference', ['user_id', 'course_id', 'waitlist_config_id'], unique=True)
