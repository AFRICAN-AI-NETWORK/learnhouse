...
def upgrade() -> None:
    ...
    if 'thumbnail_type' not in course_columns:
        type_exists = bind.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'thumbnail_type'")).scalar()
        if not type_exists:
            thumbnail_type_enum = postgresql.ENUM('IMAGE', 'VIDEO', 'BOTH', name='thumbnail_type')
            thumbnail_type_enum.create(bind)
        
        op.add_column('course', sa.Column('thumbnail_type', postgresql.ENUM('IMAGE', 'VIDEO', 'BOTH', name='thumbnail_type'), nullable=True))
