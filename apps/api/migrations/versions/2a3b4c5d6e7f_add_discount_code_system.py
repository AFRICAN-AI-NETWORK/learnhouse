...
def upgrade() -> None:
    ...
    if "discountcode" not in tables:
        ...
        op.create_index(
            op.f("ix_discountcode_code"), "discountcode", ["code"], unique=False
        )

    # Create discount_code_usage table
    if "discountcodeusage" not in tables:
        op.create_table(
            "discountcodeusage",
            sa.Column("id", sa.BigInteger(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ["discount_code_id"], ["discountcode.id"], ondelete="CASCADE"
            ),
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_discountcodeusage_user_course_unique", table_name="discountcodeusage"
    )
