...
def upgrade():
    ...
    cohort_status_enum = postgresql.ENUM(
        "UPCOMING", "ACTIVE", "COMPLETED", name="cohortstatusenum", create_type=False
    )
    ...
    if "cohort" not in tables:
        op.create_table(
            ...
            sa.Column(
                "cohort_uuid", sqlmodel.sql.sqltypes.AutoString(), nullable=False
            ),
        ...
        op.create_index(
            op.f("ix_cohort_cohort_uuid"), "cohort", ["cohort_uuid"], unique=False
        )

    # Create cohortenrollment table
    if "cohortenrollment" not in tables:
        op.create_table(
            "cohortenrollment",
            sa.Column("id", sa.Integer(), nullable=False),
            ...
            sa.Column(
                "enrollment_type", sqlmodel.sql.sqltypes.AutoString(), nullable=False
            ),
            sa.Column(
                "enrolled_date", sqlmodel.sql.sqltypes.AutoString(), nullable=False
            ),
            sa.Column("is_locked", sa.Boolean(), nullable=False),
            sa.ForeignKeyConstraint(["cohort_id"], ["cohort.id"], ondelete="CASCADE"),
            sa.ForeignKeyConstraint(["course_id"], ["course.id"], ondelete="CASCADE"),
            sa.ForeignKeyConstraint(
                ["payment_user_id"], ["paymentsuser.id"], ondelete="SET NULL"
            ),
