...
async def verify_org_admin_and_plan(
    org_id: int, request: Request, current_user: PublicUser, session: Session
):
    ...
    if not org_config:
        raise HTTPException(
            status_code=403, detail="Organization configuration not found"
        )
...
async def get_audit_logs(
    *,
    ...
    statement = (
        statement.order_by(desc(AuditLog.created_at)).offset(offset).limit(limit)
    )
    ...
    return {"items": audit_logs_read, "total": total, "limit": limit, "offset": offset}
