...
from src.db.cohorts import Cohort, CohortCreate, CohortUpdate, CohortStatusEnum, CohortEnrollment, CohortEnrollmentCreate
...
async def create_cohort(
    ...
    statement = select(Cohort).where(Cohort.org_id == cohort_data.org_id).order_by(desc(Cohort.cohort_number))
...
async def get_org_cohorts(
    org_id: int,
    db_session: Session
) -> List[Cohort]:
    statement = select(Cohort).where(Cohort.org_id == org_id).order_by(desc(Cohort.cohort_number))
    return db_session.exec(statement).all()


async def get_current_cohort(
    org_id: int,
    db_session: Session
) -> Optional[Cohort]:
    ...
    if not cohort:
        ...
        statement = select(Cohort).where(Cohort.org_id == org_id).order_by(Cohort.cohort_number.desc())
        cohort = db_session.exec(statement).first()
...
async def enroll_user_in_cohort(
    ...
    if not cohort:
        raise HTTPException(status_code=400, detail="No active cohort available for enrollment.")
