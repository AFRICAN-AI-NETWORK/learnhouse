...
async def get_course_preference_analytics(
    ...
    total_prefs_query = (
        select(func.count(func.distinct(WaitlistCoursePreference.user_id)))
        .where(WaitlistCoursePreference.waitlist_config_id == waitlist.id)
    )
    ...
    free_count = sum(1 for _, amount in free_paid_results if amount is None or amount == 0)
    paid_count = sum(1 for _, amount in free_paid_results if amount is not None and amount > 0)
