...
_domain_cache = {"disposable": set(), "legitimate": set(), "last_updated": None}
        ...
        try:
            ...
            new_cache = {
                "disposable": {
                    ...
                    if d.list_type == DomainListType.DISPOSABLE
                },
                "legitimate": {
                    d.domain
                    for d in all_domains
...
def is_cache_expired() -> bool:
    ...
    elapsed = (datetime.now(timezone.utc) - _domain_cache["last_updated"]).total_seconds()
...
async def validate_email_for_referral(
...
def _check_circuit_breaker() -> bool:
    ...
    if state == "open":
        ...
        if (
            last_failure
            and (datetime.now(timezone.utc) - last_failure).total_seconds()
            > CIRCUIT_BREAKER_TIMEOUT
        ):
            # Move to half-open state
            _circuit_breaker["state"] = "half_open"
            logger.info(
                "Circuit breaker moved to half-open state, allowing test request"
            )
            return True
        return False
...
def _record_circuit_breaker_failure(exception: Optional[Exception] = None) -> None:
    ...
    if _circuit_breaker["failures"] >= CIRCUIT_BREAKER_THRESHOLD:
        ...
        logger.warning(
            f"Circuit breaker test request failed{exception_details}, returning to open state"
        )
        _circuit_breaker["state"] = "open"
    else:
