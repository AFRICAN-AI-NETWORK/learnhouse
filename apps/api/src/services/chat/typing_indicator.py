...
class TypingIndicatorService:
    ...
    async def set_typing(
        ...
        if not conversation:
            logger.warning(f"Conversation {conversation_uuid} not found for typing indicator")
            return False
        ...
        logger.debug(f"User {user_id} typing status set to {is_typing} in conversation {conversation_uuid}")
        return True
    ...
    async def get_typing_status(
        ...
        if state.typing_updated_at:
            timeout = datetime.utcnow() - timedelta(seconds=TypingIndicatorService.TYPING_TIMEOUT_SECONDS)
