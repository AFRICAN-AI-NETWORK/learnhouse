...
try:
    ...
    logger.warning(
        "APScheduler not available, scheduled email notifications will not work"
    )
...
class NotificationService:
    ...
    @staticmethod
    async def send_message_notification(db: Session, message: Message):
        """
        ...
        await NotificationService._send_in_app_notification(message, notification)
        ...
        logger.info(
            f"Notification {notification.notification_uuid} created for message {message.message_uuid}"
        )

    @staticmethod
    async def _send_in_app_notification(
        message: Message, notification: ChatNotification
    ):
    ...
    async def _schedule_delayed_email_notification(
        db: Session, message: Message, notification: ChatNotification
    ):
        """
        ...
        if not _APSCHEDULER_AVAILABLE:
            logger.warning(
                "APScheduler not available, email notification not scheduled"
            )
    ...
    async def _check_and_send_email(message_id: int, notification_uuid: str):
        ...
        try:
            ...
            if not message or message.is_deleted:
                logger.info(
                    f"Message {message_id} not found or deleted, skipping email"
                )
                return
            ...
            if read_receipt:
                ...
                logger.info(
                    f"Message {message_id} was read, cancelling email notification"
                )

                # Update notification status
                notification = db.exec(
                    select(ChatNotification).where(
                        ChatNotification.notification_uuid == notification_uuid
                    )
                ).first()
    ...
    @staticmethod
    async def _send_email_now(db: Session, message: Message, notification_uuid: str):
            ...
            try:
                ...
                await send_email(
                    ...
                    context={
                        ...
                        "conversation_link": f"/orgs/{org.slug}/chat/{conversation.conversation_uuid}"
                        if org
                        else "#",
                ...
                logger.info(
                    f"Email notification sent for unread message {message.message_uuid}"
                )
