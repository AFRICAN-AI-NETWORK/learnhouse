...
async def handle_paystack_webhook(
    ...
    if not webhook_secret:
        ...
        raise HTTPException(
            status_code=400, detail="Paystack webhook secret not configured"
        )
    ...
    if not await verify_paystack_webhook_signature(payload, signature, webhook_secret):
        ...
        raise HTTPException(
            status_code=400, detail="Invalid Paystack webhook signature"
        )
        ...
        if event_type == "charge.success":
            ...
            try:
                ...
                if transaction_data.get("status") == "success":
                        ...
                        if payment_user and payment_user.discount_code_id:
                            try:
                                ...
                                if increment_success:
                                    ...
                                    logger.info(
                                        f"Recorded discount usage for payment_user_id: {payment_user_id}, discount_code_id: {discount_code_id}"
                                    )
                                else:
                                    logger.warning(
                                        f"Failed to increment discount usage counter (max_uses reached?) for discount_code_id: {discount_code_id}"
                                    )
                    ...
                    logger.info(
                        f"Payment completed for payment_user_id: {payment_user_id}"
                    )
                    return {
                        "status": "success",
                    ...
                    logger.warning(
                        f"Transaction verification failed for reference: {transaction_reference}"
                    )
                    return {
                        "status": "ignored",
            ...
            logger.info(
                f"Subscription cancelled for payment_user_id: {payment_user_id}"
            )
            ...
            if payment_user and payment_user.referral_code_id:
                try:
                    ...
                    commission = await forfeit_commission_for_refund(
                        payment_user_id=int(payment_user_id), db_session=db_session
                    )
