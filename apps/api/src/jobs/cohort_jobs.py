    ...
    for cohort in cohorts:
        try:
            ...
            if current_time_utc >= start_dt:
                ...
                enrollment_stmt = select(CohortEnrollment).where(CohortEnrollment.cohort_id == cohort.id)
                ...
                db_session.commit()
                logger.info(f"Successfully unlocked cohort {cohort.name} ({cohort.cohort_uuid})")
        except ValueError as e:
            logger.error(f"Error parsing start_date for cohort {cohort.cohort_uuid}: {e}")
            continue
