...
class ReferrerPayoutRequestBase(SQLModel):
    ...
    paystack_transfer_recipient_code: Optional[str] = Field(default=None, max_length=255)
    paystack_transfer_code: Optional[str] = Field(default=None, max_length=255)
    bank_account_info: dict = Field(default={}, sa_column=Column(JSON))  # Encrypted bank details
...
class ReferrerPayoutRequestCreate(SQLModel):
    ...
    org_id: int
    amount: float
    bank_details: dict  # Contains bank_name, account_number, account_holder, account_type
    currency: str = "USD"
...
class BankDetails(SQLModel):
    ...
    bank_code: Optional[str] = None  # Bank code for Paystack (e.g., "044" for Access Bank)
