...
class ReferralCommission(ReferralCommissionBase, table=True):
    ...
    __table_args__ = (
        ...
        Index("idx_referralcommission_unique", "payment_user_id", "referral_code_id", unique=True),
    )
    ...
    payment_user_id: int = Field(
        sa_column=Column(BigInteger, ForeignKey("paymentsuser.id", ondelete="CASCADE"), nullable=False)
    )
