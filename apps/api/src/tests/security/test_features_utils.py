...
class TestFeaturesUtils:
    ...
    @pytest.mark.asyncio
    async def test_check_limits_with_usage_success(
        self, mock_db_session, mock_org_config
    ):
        """Test successful feature limit check"""
        with (
            patch(
                "src.security.features_utils.usage.get_learnhouse_config"
            ) as mock_config,
            patch("redis.Redis.from_url") as mock_redis_class,
        ):
            # Mock config
            mock_config_instance = Mock()
            mock_config_instance.redis_config.redis_connection_string = (
            ...
            result = check_limits_with_usage(
                feature="ai", org_id=1, db_session=mock_db_session
            )
    ...
    @pytest.mark.asyncio
    async def test_check_limits_with_usage_feature_disabled(
        self, mock_db_session, mock_org_config
    ):
        ...
        with pytest.raises(HTTPException) as exc_info:
            check_limits_with_usage(feature="ai", org_id=1, db_session=mock_db_session)
    ...
    @pytest.mark.asyncio
    async def test_check_limits_with_usage_no_redis_connection(
        self, mock_db_session, mock_org_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_limits_with_usage_limit_reached(
        self, mock_db_session, mock_org_config
    ):
