...
class TestCalculateFraudRiskScore:
    ...
    async def test_score_for_ip_threshold_exceeded(self):
        ...
        mock_session.exec.return_value.first.return_value = (0, IP_FRAUD_THRESHOLD + 1, 0)
    ...
    async def test_score_for_device_threshold_exceeded(self):
        ...
        mock_session.exec.return_value.first.return_value = (0, 0, DEVICE_FRAUD_THRESHOLD + 1)
    ...
    async def test_temporal_window_check(self):
        ...
        await calculate_fraud_risk_score(
            "203.0.113.1",
            "device123",
            1,
            mock_session
...
class TestCreateReferralTracking:
    ...
    async def test_create_new_tracking(self):
        ...
        result = await create_referral_tracking(
...
class TestValidateAndTrackReferral:
    ...
    async def test_successful_referral_tracking(self):
        ...
        mock_session.exec.return_value.first.side_effect = [
            mock_code,  # validate_referral_code_exists
            (0, 0, 0),  # calculate_fraud_risk_score
    ...
    async def test_handles_duplicate_tracking_gracefully(self):
        ...
        mock_session.exec.return_value.first.side_effect = [
            mock_code,
            (0, 0, 0),
            existing_tracking,  # Duplicate tracking
        ]
