...
class TestCreateWaitlistUser:
    ...
    async def test_create_waitlist_user_success(
        self,
    ...
    @pytest.mark.asyncio
    async def test_create_user_with_course_preferences(
        ...
        product = PaymentsProduct(
            name="Preference Product", amount=1000, currency="USD", org_id=sample_org.id
        )
    ...
    @pytest.mark.asyncio
    async def test_create_user_with_invalid_waitlist_uuid(
    ...
    @pytest.mark.asyncio
    async def test_create_user_after_launch_date(
    ...
    async def test_create_user_duplicate_username(
        self, db_session, sample_waitlist_config, sample_org, sample_user, mock_request
    ):
    ...
    @pytest.mark.asyncio
    @patch("src.services.users.waitlist.check_limits_with_usage")
    async def test_create_user_updates_waitlist_registration_count(
        self,
        mock_check_limits,
        db_session,
        sample_waitlist_config,
...
class TestGetWaitlistUsers:
    ...
    @pytest.mark.asyncio
    async def test_get_waitlist_users(
    ...
    @pytest.mark.asyncio
    async def test_get_waitlist_users_includes_waitlist_status_only(
        self, db_session, sample_waitlist_config, sample_org, mock_request
    ):
    ...
    async def test_get_waitlist_users_invalid_uuid(self, db_session, mock_request):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_waitlist_users(mock_request, db_session, "invalid-uuid")

        assert exc_info.value.status_code == 404

    @pytest.mark.asyncio
    async def test_get_waitlist_users_empty(
        ...
        result = await get_waitlist_users(
            mock_request, db_session, empty_waitlist.waitlist_uuid
        )
