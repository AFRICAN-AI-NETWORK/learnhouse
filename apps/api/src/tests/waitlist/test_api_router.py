...
class TestWaitlistConfigEndpoints:
    ...
    async def test_create_waitlist_endpoint(
        self, mock_create, sample_org, mock_request
    ):
...
class TestWaitlistCoursesEndpoints:
    ...
    async def test_get_preferences_analytics_endpoint(
        self, mock_get_analytics, mock_request
    ):
        """Test GET /api/v1/waitlist/config/{uuid}/preferences endpoint"""
        mock_get_analytics.return_value = [
            {"product_id": 1, "product_name": "Test Course", "selection_count": 10}
        ]
...
class TestEndpointAuthorization:
    ...
    def test_admin_endpoints_require_auth(self):
        ...
        # Admin endpoints
        assert "/config" in route_paths or any(
            "/config" in path for path in route_paths
        )
...
class TestEndpointErrorHandling:
    ...
    async def test_create_waitlist_handles_errors(self, mock_create, mock_request):
        ...
        mock_create.side_effect = HTTPException(status_code=400, detail="Invalid data")
    ...
    async def test_get_waitlist_handles_not_found(self, mock_get, mock_request):
        ...
        mock_get.side_effect = HTTPException(
            status_code=404, detail="Waitlist not found"
        )
