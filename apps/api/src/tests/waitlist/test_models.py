...
class TestWaitlistEmailLogModel:
    ...
    def test_email_log_default_values(
        self, db_session, waitlist_user, sample_waitlist_config
    ):
...
class TestWaitlistCoursePreferenceModel:
    ...
    def test_create_course_preference(
        self, db_session, waitlist_user, sample_waitlist_config, sample_org
    ):
        """Test creating a course preference"""
        from src.db.payments.payments_products import PaymentsProduct

        product = PaymentsProduct(
            name="Test Product", amount=1500, currency="USD", org_id=sample_org.id
        )
    ...
    def test_multiple_preferences_same_user(
        self, db_session, waitlist_user, sample_waitlist_config, sample_org
    ):
        ...
        product1 = PaymentsProduct(
            name="Product 1", amount=1000, currency="USD", org_id=sample_org.id
        )
        product2 = PaymentsProduct(
            name="Product 2", amount=2000, currency="USD", org_id=sample_org.id
        )
...
class TestWaitlistConfigUpdate:
    ...
    def test_partial_update_model(self):
        ...
        data = WaitlistConfigUpdate(name="Updated Name")
    ...
    def test_update_launch_datetime(self):
        ...
        data = WaitlistConfigUpdate(launch_datetime=new_date)

        assert data.launch_datetime == new_date

    def test_update_status(self):
        """Test updating status"""
        data = WaitlistConfigUpdate(status=WaitlistStatusEnum.COMPLETED.value)
