...
class TestSendWaitlistConfirmationEmail:
    ...
    def test_send_confirmation_email_success(
        self, mock_send_email, sample_waitlist_config, sample_org
    ):
    ...
    @patch("src.services.waitlist.emails.send_email")
    def test_confirmation_email_contains_launch_date(
        self, mock_send_email, sample_waitlist_config, sample_org
    ):
...
class TestSendWaitlistActivationEmail:
    ...
    def test_send_activation_email_success(
        ...
        assert call_args.kwargs["to"] == user.email
        assert (
            "wait is over" in call_args.kwargs["subject"].lower()
            or "live" in call_args.kwargs["subject"].lower()
        )

    @patch("src.services.waitlist.emails.send_email")
    def test_activation_email_contains_login_info(
        self, mock_send_email, sample_waitlist_config, sample_org
    ):
...
class TestActivateWaitlist:
    ...
    async def test_activate_waitlist_sends_emails(
        self,
        mock_send_activation,
        db_session,
    ...
    @pytest.mark.asyncio
    @patch("src.services.waitlist.emails.send_waitlist_activation_email")
    async def test_activation_creates_email_logs(
        self,
        mock_send_activation,
        db_session,
        sample_waitlist_config,
    ...
    @pytest.mark.asyncio
    @patch("src.services.waitlist.emails.send_waitlist_activation_email")
    async def test_activation_handles_failures(
        self,
        mock_send_activation,
        db_session,
        sample_waitlist_config,
    ...
    @pytest.mark.asyncio
    @patch("src.services.waitlist.emails.send_waitlist_activation_email")
    async def test_activation_prevents_duplicates(
        self,
        mock_send_activation,
        db_session,
        sample_waitlist_config,
