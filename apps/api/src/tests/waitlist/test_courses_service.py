...
class TestGetOrgCoursesForWaitlist:
    ...
    @pytest.mark.asyncio
    async def test_get_courses_for_waitlist(
        self, db_session, sample_waitlist_config, sample_course, mock_request
    ):
        ...
        result = await get_org_courses_for_waitlist(
            mock_request, db_session, sample_waitlist_config.waitlist_uuid
        )
    ...
    @pytest.mark.asyncio
    async def test_free_course_pricing(
        self, db_session, sample_waitlist_config, sample_org, mock_request
    ):
        ...
        free_course_data = next(
            (c for c in result if c["course_id"] == free_course.id), None
        )
    ...
    @pytest.mark.asyncio
    async def test_paid_course_pricing(
        self, db_session, sample_waitlist_config, sample_org, mock_request
    ):
        ...
        paid_course_data = next(
            (c for c in result if c["course_id"] == paid_course.id), None
        )
    ...
    @pytest.mark.asyncio
    async def test_only_public_courses_returned(
        self, db_session, sample_waitlist_config, sample_org, mock_request
    ):
    ...
    async def test_invalid_waitlist_uuid(self, db_session, mock_request):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_org_courses_for_waitlist(mock_request, db_session, "invalid-uuid")
...
class TestGetCoursePreferenceAnalytics:
    ...
    @pytest.mark.asyncio
    async def test_get_preference_analytics(
        self,
        db_session,
    ...
    @pytest.mark.asyncio
    async def test_analytics_aggregates_multiple_users(
        self,
        db_session,
        sample_waitlist_config,
        sample_course,
        ...
        # Create product
        product = PaymentsProduct(
            name="Popular Product", amount=5000, currency="USD", org_id=sample_org.id
        )
        db_session.add(product)
        db_session.commit()
        db_session.refresh(product)
        ...
        course_data = next(
            (c for c in result["courses"] if c["product_id"] == product.id), None
        )
        assert course_data is not None
        assert course_data["selection_count"] >= 3

    @pytest.mark.asyncio
    async def test_analytics_empty_preferences(
...
class TestGetUserCoursePreferences:
    ...
    @pytest.mark.asyncio
    async def test_get_multiple_user_preferences(
        self,
        db_session,
        sample_waitlist_config,
        sample_org,
        ...
        for i in range(3):
            product = PaymentsProduct(
                name=f"Product {i}", amount=2000, currency="USD", org_id=sample_org.id
            )
            db_session.add(product)
    ...
    @pytest.mark.asyncio
    async def test_get_preferences_no_selections(
        self, db_session, sample_waitlist_config, waitlist_user, mock_request
    ):
