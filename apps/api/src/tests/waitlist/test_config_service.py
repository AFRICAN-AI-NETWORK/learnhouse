...
class TestCreateWaitlistConfig:
    ...
    @pytest.mark.asyncio
    async def test_create_valid_waitlist_config(
    ...
    @pytest.mark.asyncio
    async def test_create_waitlist_with_past_date(
    ...
    @pytest.mark.asyncio
    async def test_create_waitlist_with_invalid_datetime_format(
        self, db_session, sample_org, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_waitlist_with_custom_batch_settings(
        self, db_session, sample_org, mock_request
    ):
...
class TestGetWaitlistConfig:
    ...
    @pytest.mark.asyncio
    async def test_get_existing_waitlist_config(
...
class TestGetOrgWaitlistConfigs:
    ...
    @pytest.mark.asyncio
    async def test_get_org_waitlists(
    ...
    @pytest.mark.asyncio
    async def test_get_org_waitlists_filters_cancelled(
        self, db_session, sample_org, sample_user, mock_request
    ):
...
class TestUpdateWaitlistConfig:
    ...
    @pytest.mark.asyncio
    async def test_update_waitlist_name(
        self, db_session, sample_waitlist_config, mock_request
    ):
        ...
        result = await update_waitlist_config(
            mock_request, db_session, sample_waitlist_config.waitlist_uuid, update_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_waitlist_description(
        self, db_session, sample_waitlist_config, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_extend_launch_datetime(
        self, db_session, sample_waitlist_config, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_with_past_date_fails(
        self, db_session, sample_waitlist_config, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_batch_settings(
        self, db_session, sample_waitlist_config, mock_request
    ):
        """Test updating batch settings"""
        update_data = WaitlistConfigUpdate(batch_size=200, batch_delay_seconds=10)
    ...
    async def test_update_non_existent_waitlist(self, db_session, mock_request):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_waitlist_config(
                mock_request, db_session, "non-existent-uuid", update_data
            )
...
class TestCancelWaitlistConfig:
    ...
    @pytest.mark.asyncio
    async def test_cancel_active_waitlist(
        self, db_session, sample_waitlist_config, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_already_cancelled_waitlist(
        self, db_session, sample_org, sample_user, mock_request
    ):
        ...
        result = await cancel_waitlist_config(
            mock_request, db_session, cancelled_config.waitlist_uuid
        )
