...
class TestWaitlistActivationJob:
    ...
    def test_activation_job_closes_session_on_error(
        self, mock_session_class, mock_safe_run
    ):
...
class TestSynchronousWrappers:
    ...
    @patch(
        "src.jobs.waitlist_processor.run_waitlist_activation_job",
    ...
    @patch(
        "src.jobs.waitlist_processor.run_retry_failed_emails_job",
        new_callable=AsyncMock,
    )
    @patch("src.jobs.waitlist_processor.asyncio.run")
...
class TestWaitlistActivationIntegration:
    ...
    async def test_process_expired_waitlists(
    ...
    @pytest.mark.asyncio
    async def test_future_waitlists_not_processed(
...
class TestEmailRetryIntegration:
    ...
    async def test_retry_respects_max_attempts(
        self, db_session, sample_waitlist_config, waitlist_user
    ):
