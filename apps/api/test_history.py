...
with engine.connect() as conn:
    res = conn.execute(text("SELECT id, assignment_task_uuid, assignment_id, assignment_type FROM assignmenttask WHERE assignment_type='CODE_EDITOR' LIMIT 1")).first()
    if not res:
    ...
    # Get Assignment info
    res2 = conn.execute(text(f"SELECT activity_id, course_id, chapter_id FROM assignment WHERE id={assign_id}")).first()
    activity_id = res2[0]
    course_id = res2[1]
    chapter_id = res2[2]

    # Get Admin User ID
    res3 = conn.execute(text(f"SELECT id FROM \"user\" WHERE email='admin@school.dev'")).first()
    user_id = res3[0]
...
with engine.begin() as conn:
    ...
    sub = conn.execute(text(f"SELECT id, task_submission FROM assignmenttasksubmission WHERE user_id={user_id} AND assignment_task_id={task_id}")).first()
    ...
    if sub:
        ...
        existing_history.append({
            ...
            "submissions": [{"exerciseUUID": "ex_1", "code": f"print('Attempt {len(existing_history) + 1}')"}]
        })
        existing_json["history"] = existing_history
        
        conn.execute(text("UPDATE assignmenttasksubmission SET task_submission = :ts WHERE id = :id"), {"ts": json.dumps(existing_json), "id": sub[0]})
