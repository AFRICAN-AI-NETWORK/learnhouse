...
function TaskFormObject({
  ...
  const removeBlank = (qIndex: number, bIndex: number) => {
    ...
    if (updatedQuestions[qIndex].blanks.length > 1) {
      ...
      toast.error(
        'Cannot delete the last blank. At least one blank is required.'
      )
    }
  }
  ...
  const handleUserAnswerChange = (
    ...
    const existingIndex = updatedSubmissions.findIndex(
      (submission) =>
        ...
        submission.blankUUID === blankUUID
    )
  ...
  const handleUserAnswerBlur = (
    ...
    if (answer.trim() && view === 'student') {
      ...
      const currentIndex = allBlanks.findIndex(
        (b) => b.questionUUID === questionUUID && b.blankUUID === blankUUID
      )
      ...
      if (nextBlank) {
        setTimeout(() => {
          const nextInput = document.querySelector(
            `[data-blank-id="${nextBlank.blankUUID}"]`
          ) as HTMLInputElement
  ...
  const gradeFC = async () => {
    ...
    const finalGrade =
      totalBlanks > 0
        ? Math.round((correctAnswers / totalBlanks) * maxPoints)
        : 0
    ...
    if (res) {
      ...
      toast.success(
        `Task graded successfully with ${finalGrade} points (${correctAnswers}/${totalBlanks} correct)`
      )
    } else {
    ...
    const loadAssignmentTask = async () => {
      if (assignmentTaskUUID) {
        ...
        if (res.success) {
          ...
          if (
            res.data.contents?.questions &&
            res.data.contents.questions.length > 0
          ) {
                        ...
                        {view === 'teacher' ? (
                          <div className="flex flex-col space-y-1 w-full py-2">
                            ...
                            <input
                              ...
                              placeholder={t(
                                'dashboard.assignments.editor.task_editor.general.hint_optional'
                              )}
