...
function OrgInviteCodeGenerate(props: OrgInviteCodeGenerateProps) {
  ...
  const activeUserGroupId =
    usergroup_id || (usergroups && usergroups.length > 0 ? usergroups[0].id : 0)
  ...
  async function createInviteWithUserGroup() {
    ...
    if (res.status === 200) {
      toast.success(t('dashboard.users.signups.generate_modal.success_toast'))
      props.setInvitesModal(false)
      mutate(`${getAPIUrl()}organizations/${org.id}/invites`)
    } else {
      toast.error(t('dashboard.users.signups.generate_modal.error_toast'))
    }
  }
          ...
          <div className="flex items-center space-x-4 pt-3 mx-auto">
                ...
                <div className="">
                  ...
                  >
                    ...
                    <span>
                      {' '}
                      {t(
                        'dashboard.users.signups.generate_modal.generate_button'
                      )}{' '}
                    </span>
                  </button>
                </div>
            ...
            {usergroups?.length == 0 && (
              <div className="flex space-x-3 items-center text-xs pt-3">
                ...
                >
                  {t(
                    'dashboard.users.signups.generate_modal.create_usergroup_link'
                  )}{' '}
                </Link>
              </div>
            )}
