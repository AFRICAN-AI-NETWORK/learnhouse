...
const getValidate = (t: any) => (values: any) => {
  ...
  if (!values.name) {
    errors.name = t(
      'dashboard.users.roles.modals.create.validation.name_required'
    )
  } else if (values.name.length < 2) {
    errors.name = t(
      'dashboard.users.roles.modals.create.validation.name_min_length'
    )
  }

  if (!values.description) {
    errors.description = t(
      'dashboard.users.roles.modals.create.validation.description_required'
    )
  } else if (values.description.length < 10) {
    errors.description = t(
      'dashboard.users.roles.modals.create.validation.description_min_length'
    )
  }
...
const PermissionSection = ({
  ...
  const someSelected =
    permissions.some((perm) => sectionRights[perm]) && !allSelected
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
        ...
        >
          ...
          <span className="hidden sm:inline">
            {allSelected
              ? t(
                  'dashboard.users.roles.modals.create.permissions.deselect_all'
                )
              : t('dashboard.users.roles.modals.create.permissions.select_all')}
          </span>
          <span className="sm:hidden">
            {allSelected
              ? t('dashboard.users.roles.modals.create.permissions.deselect')
              : t('dashboard.users.roles.modals.create.permissions.select')}
          </span>
        </button>
      </div>
        ...
        {permissions.map((permission) => (
          ...
          >
            <input
              ...
              checked={
                rights[section]?.[
                  permission as keyof (typeof rights)[typeof section]
                ] || false
              }
              onChange={(e) =>
                onRightChange(section, permission, e.target.checked)
              }
              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 focus:ring-2"
            />
            <span className="text-sm text-gray-700 capitalize">
              {t(
                `dashboard.users.roles.modals.create.permissions.actions.${permission.replace('action_', '').replace(/_/g, '_')}`
              )}
            </span>
          </label>
        ))}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const toastID = toast.loading(
        t('dashboard.users.roles.modals.create.form.loading')
      )
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        toast.success(t('dashboard.users.roles.modals.create.toasts.success'), {
          id: toastID,
        })
      } else {
        setIsSubmitting(false)
        toast.error(t('dashboard.users.roles.modals.create.toasts.error'), {
          id: toastID,
        })
      }
    },
      ...
      <FormLayout onSubmit={formik.handleSubmit}>
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-4 sm:space-y-6">
            <FormField name="name">
              <FormLabelAndMessage
                label={t('dashboard.users.roles.modals.create.form.role_name')}
              ...
              <Form.Control asChild>
                <Input
                  ...
                  required
                  placeholder={t(
                    'dashboard.users.roles.modals.create.form.role_name_placeholder'
                  )}
                  className="w-full"
                />
              </Form.Control>
            ...
            <FormField name="description">
              <FormLabelAndMessage
                label={t(
                  'dashboard.users.roles.modals.create.form.description'
                )}
              ...
              <Form.Control asChild>
                <Textarea
                  ...
                  required
                  placeholder={t(
                    'dashboard.users.roles.modals.create.form.description_placeholder'
                  )}
                  className="w-full"
                />
              </Form.Control>
            ...
            <div className="mt-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                {t('dashboard.users.roles.modals.create.presets.title')}
              </h3>
                ...
                {Object.keys(predefinedRoles).map((roleKey) => (
                  ...
                  >
                    <div className="font-medium text-gray-900 text-sm sm:text-base">
                      {t(
                        `dashboard.users.roles.modals.create.presets.${roleKey === 'Course Manager' ? 'maintainer' : roleKey === 'Content Creator' ? 'content_creator' : roleKey === 'User Manager' ? 'user_manager' : roleKey.toLowerCase()}.name`
                      )}
                    </div>
                    <div className="text-xs sm:text-sm text-gray-500 mt-1">
                      {t(
                        `dashboard.users.roles.modals.create.presets.${roleKey === 'Course Manager' ? 'maintainer' : roleKey === 'Content Creator' ? 'content_creator' : roleKey === 'User Manager' ? 'user_manager' : roleKey.toLowerCase()}.description`
                      )}
                    </div>
                  </button>
                ))}
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {t('dashboard.users.roles.modals.create.permissions.title')}
            </h3>
            ...
            <PermissionSection
              ...
              t={t}
              title={t(
                'dashboard.users.roles.modals.create.permissions.sections.courses'
              )}
              icon={BookOpen}
              section="courses"
              permissions={[
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6 pt-6 border-t border-gray-200">
          ...
          <Form.Submit asChild>
            ...
            >
              {isSubmitting
                ? t('dashboard.users.roles.modals.create.form.loading')
                : t('dashboard.users.roles.modals.create.form.submit')}
            </button>
          </Form.Submit>
        </div>
      </FormLayout>
