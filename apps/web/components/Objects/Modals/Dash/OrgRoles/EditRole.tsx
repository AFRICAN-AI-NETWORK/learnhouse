...
const getValidate = (t: any) => (values: any) => {
  ...
  if (!values.name) {
    errors.name = t(
      'dashboard.users.roles.modals.edit.validation.name_required'
    )
  } else if (values.name.length < 2) {
    errors.name = t(
      'dashboard.users.roles.modals.edit.validation.name_min_length'
    )
  }

  if (!values.description) {
    errors.description = t(
      'dashboard.users.roles.modals.edit.validation.description_required'
    )
  } else if (values.description.length < 10) {
    errors.description = t(
      'dashboard.users.roles.modals.edit.validation.description_min_length'
    )
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
        ...
        >
          ...
          <span className="hidden sm:inline">
            {allSelected
              ? t('dashboard.users.roles.modals.edit.permissions.deselect_all')
              : t('dashboard.users.roles.modals.edit.permissions.select_all')}
          </span>
          <span className="sm:hidden">
            {allSelected
              ? t('dashboard.users.roles.modals.edit.permissions.deselect')
              : t('dashboard.users.roles.modals.edit.permissions.select')}
          </span>
        </button>
      </div>
        ...
        {permissions.map((permission) => (
          ...
          >
            ...
            <span className="text-sm text-gray-700 capitalize">
              {t(
                `dashboard.users.roles.modals.edit.permissions.actions.${permission.replace('action_', '').replace(/_/g, '_')}`
              )}
            </span>
          </label>
        ))}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const toastID = toast.loading(
        t('dashboard.users.roles.modals.edit.form.loading')
      )
      ...
      if (res.status === 200) {
        ...
        toast.success(t('dashboard.users.roles.modals.edit.toasts.success'), {
          id: toastID,
        })
      } else {
        setIsSubmitting(false)
        toast.error(t('dashboard.users.roles.modals.edit.toasts.error'), {
          id: toastID,
        })
      }
    },
      ...
      <FormLayout onSubmit={formik.handleSubmit}>
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-4 sm:space-y-6">
            <FormField name="name">
              <FormLabelAndMessage
                label={t('dashboard.users.roles.modals.edit.form.role_name')}
            ...
            <FormField name="description">
              <FormLabelAndMessage
                label={t('dashboard.users.roles.modals.edit.form.description')}
                ...
                {Object.keys(predefinedRoles).map((roleKey) => (
                  ...
                  >
                    <div className="font-medium text-gray-900 text-sm sm:text-base">
                      {
                        predefinedRoles[roleKey as keyof typeof predefinedRoles]
                          .name
                      }
                    </div>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {t('dashboard.users.roles.modals.edit.permissions.title')}
            </h3>
            ...
            <PermissionSection
              ...
              t={t}
              title={t(
                'dashboard.users.roles.modals.edit.permissions.sections.courses'
              )}
              icon={BookOpen}
              section="courses"
              permissions={[
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6 pt-6 border-t border-gray-200">
          ...
          <Form.Submit asChild>
            ...
            >
              {isSubmitting
                ? t('dashboard.users.roles.modals.edit.form.loading')
                : t('dashboard.users.roles.modals.edit.form.submit')}
            </button>
          </Form.Submit>
        </div>
      </FormLayout>
