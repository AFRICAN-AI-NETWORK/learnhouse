...
const WebPreviewComponent: React.FC<WebPreviewProps> = ({
  ...
  const [openInPopup, setOpenInPopup] = useState(
    node.attrs.openInPopup || false
  )
  ...
  const fetchPreview = async (url: string) => {
    ...
    try {
      ...
      const hasMinimalMetadata =
        !data.title && !data.description && !data.og_image

      if (hasMinimalMetadata) {
        toast.error(
          'Unable to get metadata from this website. The preview card may appear incomplete.',
          {
            duration: 4000,
          }
        )
