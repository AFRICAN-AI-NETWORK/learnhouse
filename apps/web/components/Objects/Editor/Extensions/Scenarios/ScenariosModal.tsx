...
const ScenariosModal: React.FC<ScenariosModalProps> = ({
  ...
  if (
    ...
    initialScenarios !== prevProps.initialScenarios ||
    initialCurrentScenarioId !== prevProps.initialCurrentScenarioId
  ) {
    setPrevProps({
      initialTitle,
      initialScenarios,
  ...
  const addNewScenario = () => {
    ...
    const newId = (
      Math.max(...scenarios.map((s) => parseInt(s.id))) + 1
    ).toString()
  ...
  const deleteScenario = (scenarioId: string) => {
    ...
    const cleanedScenarios = updatedScenarios.map((scenario) => ({
      ...
      options: scenario.options.map((option) => ({
        ...option,
        nextScenarioId:
          option.nextScenarioId === scenarioId ? null : option.nextScenarioId,
      })),
    }))
  ...
  const updateOption = (
    scenarioId: string,
    optionId: string,
    updates: Partial<ScenarioOption>
  ) => {
    const scenario = scenarios.find((s) => s.id === scenarioId)
    if (!scenario) return
  ...
  const renderPreviewContent = () => {
    ...
    return (
      <div className="flex flex-col h-[calc(75vh-220px)] p-2">
        {/* Preview Header */}
        <div className="bg-linear-to-r from-blue-50 to-indigo-50 border border-blue-200 p-4 -mx-2 -mt-2 mb-4 shrink-0">
          ...
          {previewCurrentId === 'end' ? (
            ...
            <div className="w-full max-w-xl mx-auto space-y-4 p-4">
              ...
              <div className="space-y-2">
                {previewScenario.options.map((option, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-sm font-medium shrink-0 transition-colors">
            ...
            <div className="text-center py-8 max-w-md mx-auto">
              ...
              <p className="text-slate-500 text-sm">
                Please check your scenario configuration and try again.
              </p>
            </div>
          )}
        </div>
  ...
  const renderEditContent = () => (
    <div className="flex flex-col h-[calc(75vh-220px)] p-2">
      {/* Header Section */}
      <div className="bg-white border-b border-slate-200 p-4 -mx-2 -mt-2 mb-4 shrink-0">
        ...
        <div className="h-full overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400">
          <div className="space-y-4 pb-4">
            {scenarios.map((scenario, scenarioIndex) => (
              ...
              >
                {/* Scenario Header */}
                <div className="bg-linear-to-r from-slate-50 to-slate-100 px-4 py-3 border-b border-slate-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${scenario.id === currentScenarioId ? 'bg-linear-to-r from-blue-500 to-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-500 group-hover:bg-gray-200'}`}
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      value={scenario.text}
                      onChange={(e) =>
                        updateScenario(scenario.id, { text: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {scenario.options.map((option, index) => (
                        <div
                          key={option.id}
                          className="group bg-slate-50 border border-slate-200 rounded-lg p-3 hover:bg-slate-100 transition-all"
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <button
                              onClick={() =>
                                deleteOption(scenario.id, option.id)
                              }
                              disabled={scenario.options.length <= 1}
                              className="p-1 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-all opacity-0 group-hover:opacity-100 disabled:opacity-0 disabled:cursor-not-allowed shrink-0"
                              title="Delete option"
                            >
                              <Trash2 size={12} />
                            </button>
            ...
            {scenarios.length === 0 && (
              <div className="text-center py-8">
                <div className="w-12 h-12 rounded-xl bg-orange-100 text-orange-600 flex items-center justify-center shrink-0 mx-auto mb-3">
                  <GitBranch size={20} className="text-slate-400" />
                </div>
                <h3 className="text-base font-medium text-slate-900 mb-2">
                  No scenarios yet
  ...
  return (
    <Modal
      ...
      dialogDescription={
        showPreview
          ? 'Test your interactive scenario'
          : 'Create and manage your interactive scenarios'
      }
