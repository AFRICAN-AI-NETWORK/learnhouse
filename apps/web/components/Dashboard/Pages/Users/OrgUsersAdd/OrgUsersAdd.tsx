...
function OrgUsersAdd() {
  ...
  async function sendInvites() {
    const toastId = toast.loading(
      t('dashboard.users.invite_members.toasts.sending')
    )
    setIsLoading(true)
    ...
    if (res.status == 200) {
      ...
      setIsLoading(false)
      toast.success(t('dashboard.users.invite_members.toasts.success'), {
        id: toastId,
      })
    } else {
      toast.error(t('dashboard.users.invite_members.toasts.error'), {
        id: toastId,
      })
      setIsLoading(false)
    }
  ...
  // Derived default value logic directly in render or variable
  const activeInviteCode =
    selectedInviteCode || invites?.[0]?.invite_code_uuid || ''
          ...
          <div className="ml-10 mr-10 mx-auto bg-white rounded-xl shadow-xs px-4 py-4 anit ">
            ...
            <div className="flex space-x-2 mx-auto">
              <textarea
                ...
                placeholder={t(
                  'dashboard.users.invite_members.email_placeholder'
                )}
                name=""
                id=""
              ></textarea>
            ...
            <div className="flex mx-auto my-5 ml-2 items-center space-x-4 justify-between">
              <div className="flex space-x-2 items-center">
                ...
                <ToolTip
                  content={t(
                    'dashboard.users.invite_members.invite_code_tooltip'
                  )}
                  sideOffset={8}
                  side="right"
                >
            ...
            <div className="flex flex-col bg-gray-50 -space-y-1  px-5 py-3 rounded-md mt-3 mb-3 ">
              ...
              <h2 className="text-gray-500  text-md">
                {' '}
                {t(
                  'dashboard.users.invite_members.invited_users.subtitle'
                )}{' '}
              </h2>
            </div>
                ...
                <thead className="bg-gray-100 text-gray-500 rounded-xl uppercase">
                  <tr className="font-bolder text-sm">
                    <th className="py-3 px-4">
                      {t(
                        'dashboard.users.invite_members.invited_users.table.email'
                      )}
                    </th>
                    <th className="py-3 px-4">
                      {t(
                        'dashboard.users.invite_members.invited_users.table.signup_status'
                      )}
                    </th>
                    <th className="py-3 px-4">
                      {t(
                        'dashboard.users.invite_members.invited_users.table.email_sent'
                      )}
                    </th>
                  </tr>
                </thead>
                        ...
                        <td className="py-3 px-4">
                          {invited_user.pending ? (
                            <div className="bg-orange-400 text-orange-100 w-fit px-2 py1 rounded-md">
                              {t(
                                'dashboard.users.invite_members.invited_users.status.pending'
                              )}
                            </div>
                          ) : (
                            <div className="bg-green-400 text-green-100 w-fit px-2 py1 rounded-md">
                              {t(
                                'dashboard.users.invite_members.invited_users.status.signed'
                              )}
                            </div>
                          )}
                        </td>
                        <td className="py-3 px-4">
                          {invited_user.email_sent ? (
                            <div className="bg-green-600 text-green-100 w-fit px-2 py1 rounded-md">
                              {t(
                                'dashboard.users.invite_members.invited_users.email_status.sent'
                              )}
                            </div>
                          ) : (
                            <div className="bg-red-400 text-red-100 w-fit px-2 py1 rounded-md">
                              {t(
                                'dashboard.users.invite_members.invited_users.email_status.no'
                              )}
                            </div>
                          )}
                        </td>
