...
function EditCourseContributors() {
  ...
  const [localIsOpen, setLocalIsOpen] = useState<boolean | undefined>(undefined)
  const isOpenToContributors =
    localIsOpen !== undefined
      ? localIsOpen
  ...
  const nonCreatorContributors =
    contributors?.filter((c) => c.authorship !== 'CREATOR') || []
  ...
  useEffect(() => {
    if (
      localIsOpen !== undefined &&
      courseStructure &&
      localIsOpen !== courseStructure.open_to_contributors
    ) {
      dispatchCourse({ type: 'setIsNotSaved' })
      const updatedCourse = {
        ...courseStructure,
        open_to_contributors: localIsOpen,
      }
      dispatchCourse({ type: 'setCourseStructure', payload: updatedCourse })
    }
  }, [localIsOpen, courseStructure, dispatchCourse])
  ...
  const handleAddContributors = async () => {
    ...
    try {
      const response = await bulkAddContributors(
      ...
      if (response.status === 200) {
        ...
        if (result.successful.length > 0) {
          toast.success(
            t('dashboard.courses.contributors.toasts.add_success', {
        ...
        result.failed.forEach((failure) => {
          toast.error(
            t('dashboard.courses.contributors.toasts.add_error', {
  ...
  const updateContributor = async (
    ...
    try {
      ...
      if (currentContributor.authorship === 'CREATOR') {
        toast.error(
          t('dashboard.courses.contributors.toasts.cannot_modify_creator')
        )
        return
      }
      ...
      if (res.status === 200 && res.data?.status === 'success') {
        toast.success(
          res.data.detail ||
            t('dashboard.courses.contributors.toasts.update_success')
        )
        mutate(
          `${getAPIUrl()}courses/${courseStructure.course_uuid}/contributors`
        )
        ...
        toast.error(
          t('dashboard.courses.contributors.toasts.update_error', {
            detail:
              res.data?.detail ||
              t('dashboard.courses.contributors.toasts.update_failed'),
          })
        )
      }
    } catch {
  ...
  const RoleDropdown = ({ contributor }: { contributor: Contributor }) => (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          {t(
            `dashboard.courses.contributors.roles.${contributor.authorship.toLowerCase()}`
          )}
  ...
  const StatusDropdown = ({ contributor }: { contributor: Contributor }) => (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          {t(
            `dashboard.courses.contributors.statuses.${contributor.authorship_status.toLowerCase()}`
          )}
      ...
      <DropdownMenuContent align="end" className="w-[200px]">
        {['ACTIVE', 'INACTIVE', 'PENDING'].map((status) => (
          ...
          >
            {t(
              `dashboard.courses.contributors.statuses.${status.toLowerCase()}`
            )}
  ...
  const handleBulkRemove = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success(
          t('dashboard.courses.contributors.toasts.remove_success', {
          ...
          <div className="mx-4 sm:mx-10 bg-white rounded-xl shadow-xs px-4 py-4">
            <div className="flex flex-col bg-gray-50 -space-y-1 px-3 sm:px-5 py-3 rounded-md mb-3">
              <h1 className="font-bold text-lg sm:text-xl text-gray-800">
                {t('dashboard.courses.contributors.title')}
              </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mx-auto mb-6">
              <ConfirmationModal
                ...
                confirmationMessage={t(
                  'dashboard.courses.contributors.open_to_contributors.confirmation_message'
                )}
                dialogTitle={t(
                  ...
                  <div className="relative w-full h-[200px] bg-slate-100 rounded-lg cursor-pointer hover:bg-slate-200 transition-all">
                    {isOpenToContributors && (
                      <div className="bg-green-200 text-green-600 font-bold w-fit my-3 mx-3 absolute text-sm px-3 py-1 rounded-lg">
                        {t(
                          'dashboard.courses.contributors.open_to_contributors.active'
                        )}
                      </div>
                    )}
                    <div className="flex flex-col space-y-1 justify-center items-center h-full p-2 sm:p-4">
                      ...
                      <div className="text-xl sm:text-2xl text-slate-700 font-bold">
                        {t(
                          'dashboard.courses.contributors.open_to_contributors.title'
                        )}
                      </div>
                      <div className="text-gray-400 text-sm sm:text-md tracking-tight w-full sm:w-[500px] leading-5 text-center">
                        {t(
                          'dashboard.courses.contributors.open_to_contributors.description'
                        )}
                      </div>
                    </div>
                  </div>
                }
                functionToExecute={() => setLocalIsOpen(true)}
                status="info"
              />
              <ConfirmationModal
                confirmationButtonText={t(
                  'dashboard.courses.contributors.closed_to_contributors.confirmation_button'
                )}
                confirmationMessage={t(
                  'dashboard.courses.contributors.closed_to_contributors.confirmation_message'
                )}
                dialogTitle={t(
                ...
                dialogTrigger={
                  <div className="relative w-full h-[200px] bg-slate-100 rounded-lg cursor-pointer hover:bg-slate-200 transition-all">
                    {!isOpenToContributors && (
                      <div className="bg-green-200 text-green-600 font-bold w-fit my-3 mx-3 absolute text-sm px-3 py-1 rounded-lg">
                        {t(
                          'dashboard.courses.contributors.closed_to_contributors.active'
                        )}
                      </div>
                    )}
                    <div className="flex flex-col space-y-1 justify-center items-center h-full p-2 sm:p-4">
                      ...
                      <div className="text-xl sm:text-2xl text-slate-700 font-bold">
                        {t(
                          'dashboard.courses.contributors.closed_to_contributors.title'
                        )}
                      </div>
                      <div className="text-gray-400 text-sm sm:text-md tracking-tight w-full sm:w-[500px] leading-5 text-center">
                        {t(
                          'dashboard.courses.contributors.closed_to_contributors.description'
                        )}
                      </div>
                    </div>
                  </div>
                }
                functionToExecute={() => setLocalIsOpen(false)}
                status="info"
              />
            </div>
            <div className="space-y-4">
              <div className="relative">
                ...
                <Input
                  placeholder={t(
                    'dashboard.courses.contributors.search.placeholder'
                  )}
                  value={searchQuery}
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-700">
                              {selectedUsers.length === 1
                                ? t(
                                    'dashboard.courses.contributors.actions.selected_users',
                                    { count: selectedUsers.length }
                                  )
                                : t(
                                    'dashboard.courses.contributors.actions.selected_users_plural',
                                    { count: selectedUsers.length }
                                  )}
                            </span>
                            <div className="flex gap-2">
                              ...
                              >
                                {t(
                                  'dashboard.courses.contributors.actions.clear'
                                )}
                              </Button>
                              <Button
                              ...
                              >
                                {t(
                                  'dashboard.courses.contributors.actions.add_selected'
                                )}
                              </Button>
                            </div>
                          </div>
                          ...
                          >
                            ...
                            {isExistingContributor && (
                              <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">
                                {t(
                                  'dashboard.courses.contributors.actions.already_contributor'
                                )}
                              </span>
                            )}
                          </div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">
                        {selectedContributors.length === 1
                          ? t(
                              'dashboard.courses.contributors.actions.selected_contributors',
                              { count: selectedContributors.length }
                            )
                          : t(
                              'dashboard.courses.contributors.actions.selected_contributors_plural',
                              { count: selectedContributors.length }
                            )}
                      </span>
                      <div className="flex gap-2">
                        ...
                        >
                          {t(
                            'dashboard.courses.contributors.actions.remove_selected'
                          )}
                        </Button>
                      </div>
                    </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                            ...
                            onChange={(e) => {
                              if (contributors) {
                                if (e.target.checked) {
                                  ...
                                  const nonCreatorContributorsIds = contributors
                                    .filter((c) => c.authorship !== 'CREATOR')
                                    .map((c) => c.user_id)
                                  setSelectedContributors(
                                    nonCreatorContributorsIds
                                  )
                                } else {
                                  setSelectedContributors([])
                                }
                        ...
                        <TableHead>
                          {t('dashboard.courses.contributors.table.name')}
                        </TableHead>
                        <TableHead>
                          {t('dashboard.courses.contributors.table.username')}
                        </TableHead>
                        <TableHead>
                          {t('dashboard.courses.contributors.table.email')}
                        </TableHead>
                        <TableHead>
                          {t('dashboard.courses.contributors.table.role')}
                        </TableHead>
                        <TableHead>
                          {t('dashboard.courses.contributors.table.status')}
                        </TableHead>
                        <TableHead>
                          {t('dashboard.courses.contributors.table.added_on')}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
