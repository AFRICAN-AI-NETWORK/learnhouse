...
const OrgAuditLogs = () => {
  ...
  const logsUrl =
    org && access_token ? `${getAPIUrl()}ee/audit_logs/?${buildQuery()}` : null
  const { data, isLoading, isValidating } = useSWR(logsUrl, (url) =>
  ...
  const handleExport = async () => {
    ...
    const toastId = toast.loading(
      t('dashboard.organization.audit_logs.export_toasts.generating')
    )
    try {
      const params = new URLSearchParams()
      if (org?.id) params.append('org_id', org.id.toString())
      ...
      a.remove()
      toast.success(
        t('dashboard.organization.audit_logs.export_toasts.success'),
        { id: toastId }
      )
    } catch (error) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center space-y-4">
        ...
        <h1 className="text-2xl font-bold text-gray-800">
          {t('dashboard.organization.audit_logs.enterprise_feature')}
        </h1>
            ...
            <div className="flex gap-2 items-center">
              ...
              >
                ...
                <span className="hidden md:inline font-medium">
                  {t('dashboard.organization.audit_logs.export')}
                </span>
              </button>
              ...
              <div className="flex items-center gap-0 border border-gray-200 rounded-md overflow-hidden bg-white">
                ...
                >
                  <SelectTrigger className="w-[110px] h-9 text-[10px] border-none bg-gray-50 rounded-none focus:ring-0">
                    <SelectValue
                      placeholder={t(
                        'dashboard.organization.audit_logs.search_by'
                      )}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="action">
                      {t('dashboard.organization.audit_logs.table.path_method')}
                    </SelectItem>
                    <SelectItem value="resource">
                      {t('dashboard.organization.audit_logs.filters.resource')}
                    </SelectItem>
                    <SelectItem value="username">
                      {t('dashboard.organization.audit_logs.table.user')}
                    </SelectItem>
                    ...
                    <SelectItem value="ip_address">
                      {t('dashboard.organization.audit_logs.table.ip_address')}
                    </SelectItem>
                  </SelectContent>
                </Select>
                <div className="relative">
                  ...
                  <Input
                    placeholder={t(
                      'dashboard.organization.audit_logs.placeholders.search'
                    )}
              ...
              >
                <SelectTrigger className="w-[120px] h-9 text-sm">
                  <SelectValue
                    placeholder={t(
                      'dashboard.organization.audit_logs.filters.resource'
                    )}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t('dashboard.organization.audit_logs.resources.all')}
                  </SelectItem>
                  <SelectItem value="user">
                    {t('dashboard.organization.audit_logs.resources.user')}
                  </SelectItem>
                  <SelectItem value="course">
                    {t('dashboard.organization.audit_logs.resources.course')}
                  </SelectItem>
                  <SelectItem value="org">
                    {t('dashboard.organization.audit_logs.resources.org')}
                  </SelectItem>
                </SelectContent>
              </Select>
              ...
              >
                <SelectTrigger className="w-[100px] h-9 text-sm">
                  <SelectValue
                    placeholder={t(
                      'dashboard.organization.audit_logs.filters.status'
                    )}
                  />
                </SelectTrigger>
                <SelectContent>
              ...
              >
                <SelectTrigger className="w-[130px] h-9 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <SelectValue
                      placeholder={t(
                        'dashboard.organization.audit_logs.filters.date'
                      )}
                    />
                  </div>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t('dashboard.organization.audit_logs.date_ranges.all')}
                  </SelectItem>
                  <SelectItem value="today">
                    {t('dashboard.organization.audit_logs.date_ranges.today')}
                  </SelectItem>
                  <SelectItem value="7d">
                    {t('dashboard.organization.audit_logs.date_ranges.7d')}
                  </SelectItem>
                  <SelectItem value="30d">
                    {t('dashboard.organization.audit_logs.date_ranges.30d')}
                  </SelectItem>
                  <SelectItem value="custom">
                    {t('dashboard.organization.audit_logs.date_ranges.custom')}
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          <thead className="bg-gray-100 text-gray-500 rounded-xl uppercase">
            <tr className="font-bolder text-[10px] tracking-wider">
              <th className="py-3 px-4">
                {t('dashboard.organization.audit_logs.table.timestamp')}
              </th>
              <th className="py-3 px-4">
                {t('dashboard.organization.audit_logs.table.user')}
              </th>
              <th className="py-3 px-4">
                {t('dashboard.organization.audit_logs.table.resource')}
              </th>
              <th className="py-3 px-4">
                {t('dashboard.organization.audit_logs.table.path_method')}
              </th>
              ...
              <th className="py-3 px-4 text-right">
                {t('dashboard.organization.audit_logs.table.status')}
              </th>
              <th className="py-3 px-4 text-right">
                {t('dashboard.organization.audit_logs.table.payload')}
              </th>
            </tr>
          </thead>
          <tbody className="bg-white relative">
            {isLoading && !data ? (
                ...
                <td colSpan={7} className="py-20 text-center text-gray-400">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-sm font-medium">
                      {t('dashboard.organization.audit_logs.loading')}
                    </span>
                  </div>
                </td>
              </tr>
              ...
              logs.map((log: any) => (
                <tr
                  key={log.id}
                  className="border-b border-gray-200 border-dashed hover:bg-gray-50/50 transition-colors"
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <span
                        className={`w-fit text-[9px] font-bold px-1.5 py-0.5 rounded ${
                          log.method === 'GET'
                  ...
                  <td className="py-3 px-4 text-right">
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${getStatusColor(log.status_code)}`}
                    >
                      {log.status_code}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right">
                    {log.payload && Object.keys(log.payload).length > 0 ? (
                      <Modal
                        dialogTitle={t(
                          'dashboard.organization.audit_logs.modals.payload.title'
                        )}
                        dialogDescription={t(
                          'dashboard.organization.audit_logs.modals.payload.description',
                          { action: log.action }
                        )}
                        minHeight="no-min"
                        isDialogOpen={openModalId === log.id}
                        onOpenChange={(open) =>
                          setOpenModalId(open ? log.id : null)
                        }
                        dialogContent={
        ...
        {total > ITEMS_PER_PAGE && (
          <div className="mt-4 flex items-center justify-between px-2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  handlePageChange(Math.max(0, offset - ITEMS_PER_PAGE))
                }
                disabled={offset === 0}
