...
- `create_flutterwave_transfer_recipient` hard-codes `NIGERIAN_BANK`: fails for all non-Nigerian accounts
...
        │
        ▼
Student pays (Flutterwave webhook fires)
        │
        ▼
...
Flutterwave transfer fires automatically
        │
        ▼
...
- `flutterwave_beneficiary_id` (str, 255, nullable) — cached after first-time recipient

creation. Reused on subsequent payouts to avoid redundant Flutterwave API calls.
...
**\#### 3.2.3** **`create_flutterwave_transfer_recipient`** **Hard-Codes Nigerian Bank Type**

**File:** `apps/api/src/services/referrals/payouts.py` — `create_flutterwave_transfer_recipient`

**Root cause:** The function hard-codes `"type": "NIGERIAN_BANK"` which is only valid for Nigerian
bank accounts. Any payout to Ghana, Kenya, South Africa, Rwanda, or Tanzania fails at the
Flutterwave API level with a recipient validation error.

**Fix:** Add `CURRENCY_TO_PAYSTACK_RECIPIENT_TYPE` map to this file:

```plain
NGN → NIGERIAN_BANK          (Nigerian bank)
GHS → ghipss         (Ghanaian bank)
KES → mobile_money   (Kenya M-Pesa via Flutterwave)
ZAR → basa           (South African bank)
RWF → mobile_money   (Rwanda mobile money)
TZS → mobile_money   (Tanzania mobile money)
XOF → mobile_money   (West Africa CFA — Flutterwave limited support)
EGP → NIGERIAN_BANK          (Egypt — Flutterwave Nile)
USD → NIGERIAN_BANK          (international)
```

Update `create_flutterwave_transfer_recipient` signature to accept `payment_method_type`
(BANK_TRANSFER or MOBILE_MONEY) and `currency`. Replace the hard-coded `"type": "NIGERIAN_BANK"`
with `CURRENCY_TO_PAYSTACK_RECIPIENT_TYPE.get(currency, "NIGERIAN_BANK")`. For MOBILE_MONEY type,
...
even if both Flutterwave and Flutterwave webhooks fire for the same payment — no extra
...
- Creates new record with `is_active = True`, `flutterwave_beneficiary_id = None`
- If marketer's country changes in their user profile later, a post-save hook in the user

profile update endpoint calls this reset: `flutterwave_beneficiary_id = None` on all methods

**`cache_flutterwave_beneficiary_id(payment_method_id, recipient_code, db_session) -> None`**
Writes `flutterwave_beneficiary_id` to the `MarketerPaymentMethod` row after first creation.
On subsequent payouts, `process_payout_request` checks: if `payment_method.flutterwave_beneficiary_id`
is not None, skip the Flutterwave `/transferrecipient` call and use the cached code directly.
...
- On Flutterwave failure: increment `payout.retry_count`, set `payout.last_retry_at = now()`
...
- Saves marketer from a permanent-fail on a transient Flutterwave error
...
- `MKTR_306` — Flutterwave recipient creation failed — check your bank details
...
- `MKTR_354` — Account details invalid (Flutterwave verification failed)
...
`process_payout_request` (after fix 3.2.1) on Flutterwave API failure:
...
- `marketer_payout_completed.html` — sent on COMPLETED; includes amount, local currency equivalent, last 4 digits, Flutterwave reference
...
  — FIX: replace NIGERIAN_BANK hard-code with CURRENCY_TO_PAYSTACK_RECIPIENT_TYPE map
...
  — add cache_flutterwave_beneficiary_id()
...
On submit: calls `savePaymentMethod`. On `MKTR_354` (Flutterwave validation failed): shows
...
- Cached recipient code reused when `flutterwave_beneficiary_id` is set
- New recipient created when `flutterwave_beneficiary_id` is None
- Mobile money recipient uses phone_number not account_number
- `check_pending_payout` blocks when APPROVED payout exists
- Retry count increments on Flutterwave failure; FAILED after 3 retries
...
Flutterwave fix (3.2.6), exchange rate fix (3.2.4), NIGERIAN_BANK fix (3.2.3), country expansion
...
| Flutterwave recipient creation  | `create_flutterwave_transfer_recipient()` in `payouts.py`             |
