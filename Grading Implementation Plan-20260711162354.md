...
(auto-graded for QUIZ, FORM, CODE\_EDITOR types; manually graded for FILE\_SUBMISSION and
...
**How points\_earned is derived per activity type:**

For activities that have an associated assignment (types ASSIGNMENT, with tasks of type QUIZ,
FORM, CODE\_EDITOR, FILE\_SUBMISSION, or OTHER):
...
SMART\_ARTICLE, LIVE\_SESSION, ATTENDANCE, CUSTOM):
...
*   If `sum(task.max_grade_value)` is 0 for an assignment (all tasks have max\_grade\_value of
...
*   Includes a descriptive message such as "add grade\_percentage to certificateuser".
